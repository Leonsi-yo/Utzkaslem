...
import { Sprout, CalendarDays, RefreshCw, Settings } from "lucide-react"
...
import { SmsConfigDialog } from "./SmsConfig"
...
export function Sidebar({
  ...
  const availableCrops: Crop[] = selectedDepartmentName ? getCropsByDepartment(selectedDepartmentName) : allCrops
    ...
    <aside className="w-full lg:w-80 bg-white border-b lg:border-r lg:border-b-0 border-blue-200 shadow-lg p-4 lg:p-6 overflow-y-auto">
      ...
      <div className="space-y-5">
        <Card className="p-5 bg-gradient-to-br from-green-50 to-green-100 border-2 border-green-300 shadow-md">
          ...
          {selectedDepartmentName && (
            <p className="text-sm text-green-700 mb-2">📍 Cultivos de {selectedDepartmentName}</p>
          )}
          ...
          {selectedDepartmentName && availableCrops.length > 0 && (
            <p className="text-xs text-green-600 mt-2">
              {availableCrops.length} cultivo{availableCrops.length !== 1 ? "s" : ""} disponible
              {availableCrops.length !== 1 ? "s" : ""}
            </p>
          )}
        </Card>
          ...
          <Select value={selectedRecommendation} onValueChange={onRecommendationChange}>
            ...
            <SelectContent>
              <SelectItem value="siembra" className="text-lg py-3">
                <span className="flex items-center gap-2">🌱 Siembra</span>
              </SelectItem>
              <SelectItem value="cosecha" className="text-lg py-3">
                <span className="flex items-center gap-2">🌾 Cosecha</span>
              </SelectItem>
              <SelectItem value="fumigacion" className="text-lg py-3">
                <span className="flex items-center gap-2">💨 Fumigación</span>
              </SelectItem>
            </SelectContent>
          </Select>
        </Card>
        ...
        <Card className="p-5 bg-gradient-to-br from-blue-50 to-cyan-100 border-2 border-blue-300 shadow-md">
          <div className="flex items-center gap-2 mb-3">
            ...
            <Label className="text-blue-900">📅 Ventana de Tiempo</Label>
          </div>
        ...
        <Card className="p-5 bg-gradient-to-br from-orange-50 to-orange-100 border-2 border-orange-300 shadow-md">
          <div className="flex items-center justify-between mb-3">
            <Label className="text-orange-900">📱 Alertas SMS</Label>
            <button
      ...
      {showSmsConfig && <SmsConfigDialog onClose={() => setShowSmsConfig(false)} />}
    </aside>
  )
}
